...
namespace Addons\Room\Controller;
...
class RoomController extends AddonsController{
    ...
    {
        parent::__construct();
        $this->model = M('Model')->getByName('room');
    ...
    function lists() {
        ...
        foreach($data as $i=>$d) {
            ...
            $data[$i]['updated_at'] = date("Y-m-d H:i",$d['updated_at']);
        }
        /* 查询记录总数 */
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            if(time()-7*24*3600 > strtotime($_POST['start_time']))
                $this->error("只能预订七天后的时间");
